...
- [x] **T111 — Paginate PDF exports on DIN A4 paper** (`FR-6`, `B017`): Specify A4 output, print the canonical HTML with explicit paper size and margins, preserve content across page breaks, and verify short and long publications with PDFKit.
- [x] **T112 — Restore a visible one-click export action** (`B018`): Replace the collapsing compact-menu group with an explicit primary button and options disclosure; extend export confirmation to ten seconds and verify toolbar export end to end.
- [x] **T113 — Give history a readable startup width** (`B019`): Enforce a 240–360 point history column with a 280-point ideal width at the split-view boundary and verify launch sizing.
- [x] **T114 — Simplify export actions and polish A4 publications**: Show HTML and PDF as immediate menu actions with Theme as the only preference; use compact print typography, 12 mm content margins, and edge-to-edge theme paper while preserving text and links. PDF regression and visual checks passed; manual menu check remains recorded under B020 because Xcode could not activate its UI-test app.
- [x] **E073 — Use the implementation-relative exporter path**: Correct a patch path that omitted implementations/mac before applying the export changes.
- [x] **E074 — Avoid nested Swift Testing macros**: Split bitmap-data and image assertions into separate require calls so the publication regression checks compile.
- [x] **E075 — Account for native print font metrics**: Validate PDF body fonts within one point of the nominal 10.5-point CSS size, allowing WebKit's print shrink factor while rejecting screen-sized text.
- [x] **E076 — Match the PDF underlay to WebKit's color space**: Use explicit sRGB for CSS-derived paper colors and validation; device RGB caused the margins to look brighter than the printed content.
- [x] **E077 — Disambiguate color component conversion**: Use an explicit Double-to-CGFloat closure when constructing the sRGB paper color.
- [x] **E078 — Respect theme capitalization in export checks**: Match the publication heading case-insensitively because the Sci-Fi theme intentionally uppercases headings.
- [x] **E079 — Remove stray controls and improve printed code contrast**: Generate Copy controls only after sanitization so their stripped text cannot leak into publications; use readable theme colors for code tokens on dark paper.
- [x] **E071 — Correct documentation patch anchors**: Re-read the bug tracker's heading after a patch-context mismatch and reapply the changes against the actual document structure.
- [x] **E072 — Query export feedback by its native accessibility role**: The UI test confirmed sidebar sizing and one-click PDF creation but queried Reveal in Finder as a button; use its actual Link role and expose the full success message to accessibility even when visually truncated.
- [x] **E070 — Respect WebKit's asynchronous printing lifecycle**: Run PDF print jobs asynchronously with serialized access; receive AppKit's background-thread completion through a nonisolated callback and resume on the main actor. Targeted export tests verify completion, A4 dimensions, pagination, preserved content, and filename collision handling.
